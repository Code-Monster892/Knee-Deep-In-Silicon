module cpu (
    ...
    be be_inst (
        ...
        .write_data(reg_data2),     // Raw data from register
        .mask(mem_mask),
        .shifted_data(shifted_write_data)
    );

    // --- MMIO Assignments ---
    assign mmio_we = mem_write & (alu_result >= 32'h01000000);
    assign mmio_read_en = ~mem_write & (alu_result >= 32'h01000000);
    assign mmio_address = alu_result;
    assign mmio_write_data = reg_data2;
    ...
    memory mem_inst (
        .clk(clk),
        .rst_n(rst_n),
        .we(mem_write & (alu_result < 32'h01000000)), // Only write to RAM if address < 0x01000000
        .mask(mem_mask),
        .address(alu_result),      // Data Address from ALU
        .write_data(shifted_write_data),    // New Shifted Data
        .read_data(ram_read_data), // This becomes raw_read_data for the reader
        .pc_address(pc),           // Instruction Address
        .instruction(instr)        // Instruction read
    );

    // Multiplex read data between RAM and MMIO
    assign mem_read_data = (alu_result >= 32'h01000000) ? mmio_read_data : ram_read_data;
    ...
    always_comb begin
        case(result_src)
            ...
            3'b001: writeback_data = final_read_data;  //Track 1: Extracted Memory load
