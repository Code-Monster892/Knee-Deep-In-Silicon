module control (
    ...
    always_comb begin
        ...
        case (op)
            ...
            7'b1100011: begin // B-Type Instruction (beq, bne, etc)
                reg_write  = 1'b0;
                imm_src    = 2'b10;  // Use the B-Type immediate!
                mem_write  = 1'b0;
                alu_src    = 1'b0;   // ALU takes 'rs2' to compare them
                result_src = 1'b0;
                branch     = 1'b1;   // this is a branch instruction!
                alu_op     = 2'b01;  // Tell ALU Decoder to Subtract
            end
        endcase
    end
    // 3. ALU Decoder
